...
export async function GET(
    ...
    try {
        ...
        console.log(`Flat Download Request: ${requestedFilename} for Session: ${sessionId}`);
        ...
        if (!session || !session.signedAgreement) {
            return new NextResponse('Waiver not found or not signed', { status: 404 });
        }

        const sa = session.signedAgreement;

        // --- PDF Generation Logic (Duplicated for Safety) ---
        // Ideally checking for "buffer" availability 
        // Note: In Next.js Node runtime, Buffer is available globally or via import. We imported it above.
        ...
        let filename = 'waiver.pdf';
        try {
            filename = decodeURIComponent(requestedFilename);
        } catch {
        ...
        console.error('Server Document Generation Error:', error);
